<template><div><h1 id="ssh-otp-redhat-계열" tabindex="-1"><a class="header-anchor" href="#ssh-otp-redhat-계열" aria-hidden="true">#</a> SSH OTP - RedHat 계열</h1>
<blockquote>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>vault-ssh-helper 구성 &lt;config.hcl&gt;</p>
<blockquote>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>vault-ssh-helper 다운로드</p>
<blockquote>
<p><a href="https://releases.hashicorp.com/vault-ssh-helper/" target="_blank" rel="noopener noreferrer">https://releases.hashicorp.com/vault-ssh-helper/<ExternalLinkIcon/></a></p>
</blockquote>
<p>vault-ssh-helper verify</p>
<blockquote>
<p>tls를 사용하지 않는 경우. <code v-pre>-dev</code></p>
</blockquote>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>Pam 설정</p>
<p><code v-pre>/etc/pam.d/sshd</code> 파일의 <code v-pre>@include common-auth</code> 부분을 다음과 같이 변경 추가</p>
<div class="language-properties line-numbers-mode" data-ext="properties"><pre v-pre class="language-properties"><code><span class="token comment">#%PAM-1.0</span>
<span class="token key attr-name">auth</span> <span class="token value attr-value">       required    pam_sepermit.so</span>
<span class="token comment">#auth       substack    password-auth # COMMENT OUT FOR SSH-HELPER</span>
<span class="token key attr-name">auth</span> <span class="token value attr-value">       include     postlogin</span>
<span class="token key attr-name">auth</span> <span class="token value attr-value">       requisite   pam_exec.so quiet expose_authtok log=/var/log/vaultssh.log /usr/local/bin/vault-ssh-helper -config=/etc/vault-ssh-helper.d/config.hcl -dev</span>
<span class="token key attr-name">auth</span> <span class="token value attr-value">       optional    pam_unix.so not_set_pass use_first_pass nodelay</span>
<span class="token comment"># Used with polkit to reauthorize users in remote sessions</span>
<span class="token key attr-name">-auth</span> <span class="token value attr-value">      optional    pam_reauthorize.so prepare</span>
<span class="token key attr-name">account</span> <span class="token value attr-value">    required    pam_nologin.so</span>
<span class="token key attr-name">account</span> <span class="token value attr-value">    include     password-auth</span>
<span class="token comment">#password   include     password-auth # COMMENT OUT FOR SSH-HELPER</span>
<span class="token comment"># pam_selinux.so close should be the first session rule</span>
<span class="token key attr-name">session</span> <span class="token value attr-value">    required    pam_selinux.so close</span>
<span class="token key attr-name">session</span> <span class="token value attr-value">    required    pam_loginuid.so</span>
<span class="token comment"># pam_selinux.so open should only be followed by sessions to be executed in the user context</span>
<span class="token key attr-name">session</span> <span class="token value attr-value">    required    pam_selinux.so open env_params</span>
<span class="token key attr-name">session</span> <span class="token value attr-value">    required    pam_namespace.so</span>
<span class="token key attr-name">session</span> <span class="token value attr-value">    optional    pam_keyinit.so force revoke</span>
<span class="token key attr-name">session</span> <span class="token value attr-value">    include     password-auth</span>
<span class="token key attr-name">session</span> <span class="token value attr-value">    include     postlogin</span>
<span class="token comment"># Used with polkit to reauthorize users in remote sessions</span>
<span class="token key attr-name">-session</span> <span class="token value attr-value">  optional     pam_reauthorize.so prepare</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>ssh 설정</p>
<p><code v-pre>/etc/ssh/sshd_config</code> 파일의 다음 항목을 수정</p>
<blockquote>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>ssh 서비스 재시작</p>
...
Password:
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>접속 방법 2. vault ssh</p>
<blockquote>
