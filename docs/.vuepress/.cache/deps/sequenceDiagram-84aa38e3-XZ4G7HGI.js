...
// node_modules/mermaid/dist/sequenceDiagram-84aa38e3.js
...
var addSignal = function(idFrom, idTo, message = { text: void 0, wrap: void 0 }, messageType) {
...
var parseBoxData = function(str) {
  ...
  const boxData = {
    color,
    text: title !== void 0 ? sanitizeText$2(title.replace(/^:?(?:no)?wrap:/, ""), getConfig$1()) : void 0,
...
var addLinks = function(actorId, text) {
  ...
  try {
    let sanitizedText = sanitizeText$2(text.text, getConfig$1());
...
var addPopupInteraction = (id, actorCnt2) => {
  addFunction(() => {
...
var actorCnt = -1;
var fixLifeLineHeights = (diagram2, actors2, actorKeys, conf2) => {
  if (!diagram2.select) {
    return;
  }
...
var drawActorTypeParticipant = function(elem, actor, conf2, isFooter) {
...
var drawActorTypeActor = function(elem, actor, conf2, isFooter) {
  const actorY = isFooter ? actor.stopy : actor.starty;
  const center = actor.x + actor.width / 2;
...
var drawActor = function(elem, actor, conf2, isFooter) {
  switch (actor.type) {
    case "actor":
...
var drawBox = function(elem, box, conf2) {
...
var drawNote = function(elem, noteModel) {
...
var drawMessage = function(diagram2, msgModel, lineStartY, diagObj) {
...
var addActorRenderingData = function(diagram2, actors2, createdActors2, actorKeys, verticalPos, messages2, isFooter) {
  let prevWidth = 0;
  let prevMargin = 0;
  let prevBox = void 0;
  let maxHeight = 0;
...
var drawActors = function(diagram2, actors2, actorKeys, isFooter) {
  if (!isFooter) {
    for (const actorKey of actorKeys) {
      const actor = actors2[actorKey];
...
var drawActorsPopup = function(diagram2, actors2, actorKeys, doc) {
  let maxHeight = 0;
  let maxWidth = 0;
  for (const actorKey of actorKeys) {
    const actor = actors2[actorKey];
  ...
  if (msg.placement === diagObj.db.PLACEMENT.RIGHTOF) {
    ...
    textDimensions = utils.calculateTextDimensions(
      shouldWrap ? utils.wrapLabel(
        msg.message,
        common$1.getMax(conf.width, actors2[msg.from].width),
        noteFont(conf)
      ) : msg.message,
      noteFont(conf)
    );
    noteModel.width = shouldWrap ? common$1.getMax(conf.width, actors2[msg.from].width) : common$1.getMax(
...
//# sourceMappingURL=sequenceDiagram-84aa38e3-XZ4G7HGI.js.map
