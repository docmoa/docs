import {
  ...
  parser$1,
  styles
} from "./chunk-HB3AAVE5.js";
import {
  Graph,
  layout
} from "./chunk-ODTBTE3U.js";
...
// node_modules/mermaid/dist/stateDiagram-9a586ac6.js
...
var drawDescrState = (g, stateDef) => {
  ...
  const title = g.append("text").attr("x", 2 * getConfig$1().state.padding).attr("y", getConfig$1().state.textHeight + 1.3 * getConfig$1().state.padding).attr("font-size", getConfig$1().state.fontSize).attr("class", "state-title").text(stateDef.descriptions[0]);
  const titleBox = title.node().getBBox();
  const titleHeight = titleBox.height;
  const description = g.append("text").attr("x", getConfig$1().state.padding).attr(
    "y",
...
var addTitleAndBox = (g, stateDef, altBkg) => {
  ...
  const orgX = orgBox.x;
  const title = g.append("text").attr("x", 0).attr("y", getConfig$1().state.titleShift).attr("font-size", getConfig$1().state.fontSize).attr("class", "state-title").text(stateDef.id);
  const titleBox = title.node().getBBox();
...
var drawNote = (text, g) => {
  ...
  const note = g.append("rect").attr("x", 0).attr("y", getConfig$1().state.padding);
  const rectElem = g.append("g");
  const { textWidth, textHeight } = _drawLongText(text, 0, 0, rectElem);
  note.attr("height", textHeight + 2 * getConfig$1().state.noteMargin);
  note.attr("width", textWidth + getConfig$1().state.noteMargin * 2);
  return note;
};
...
//# sourceMappingURL=stateDiagram-9a586ac6-7VB6PQJP.js.map
