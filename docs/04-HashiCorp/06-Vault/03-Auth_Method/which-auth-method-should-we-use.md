...
- **SPIFFE (`spiffe` auth, Enterprise)**
  - SPIRE 등으로 발급한 **SVID**(JWT 또는 X.509)로 인증; SPIFFE ID를 Vault 역할·정책에 매핑
  - Vault는 SVID **발급자가 아니라 검증자**이며, SPIRE와 병행하는 구성이 일반적임 ([SPIFFE 인증](spiffe-auth.md), 배경 개념은 [SPIFFE/SPIRE 개요](../../../01-Infrastructure/Security/spiffe-spire-overview.md))
  - 사용 예: 워크로드 ID와 시크릿 접근 정책을 동일 모델로 맞추고 싶을 때
...
| 적용 대상                | 권장 인증 방식                                      |
|------------------------|--------------------------------------------------|
| 일반 애플리케이션, 클라우드 환경   | TLS Certificate, AppRole, AWS IAM, Azure Auth Method, GCP Auth Method, OCI Auth Method, Kubernetes Auth, SPIFFE Auth (Enterprise) |
