...
::: tip
프롬프트 엔지니어링은 LLM의 능력을 최대한 활용하고 할루시네이션을 제어하기 위한 핵심 기술이다. 올바른 프롬프트 설계를 통해 LLM의 창조적 특성을 활용하면서도 신뢰할 수 있는 결과를 얻을 수 있다.
:::
...
프롬프트 엔지니어링(Prompt Engineering)은 대규모 언어 모델(Large Language Model, LLM)과 효과적으로 상호작용하기 위해 프롬프트를 설계하고 최적화하는 기술이다. 프롬프트는 LLM에게 전달하는 입력 텍스트로, 모델이 원하는 출력을 생성하도록 지시하는 역할을 한다.
...
대규모 언어 모델은 수조 개의 파라미터를 가진 신경망으로, 방대한 양의 텍스트 데이터로 학습된다. 
...
할루시네이션은 LLM이 학습 데이터에 없거나 사실과 다른 정보를 생성하는 현상이다. 이는 단순한 버그가 아니라 LLM의 근본적인 특성이다.
